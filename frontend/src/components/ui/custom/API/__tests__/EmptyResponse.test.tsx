...
describe("EmptyResponse", () => {
  it("renders proper role, aria-live, aria-labelledby", () => {
    render(<EmptyResponse labelledBy="Test label">Oops..</EmptyResponse>);

    const message = screen.getByTestId("empty-reponse");

    expect(message).toHaveAttribute("role", "status");
    expect(message).toHaveAttribute("aria-live", "polite");
    expect(message).toHaveAttribute("aria-labelledby", "Test label");
  });
